...
export const useExcel = () => {
	...
	// 分批处理数据
	const processBatch = (worksheet, rowCount, colCount, startRow, startCol, batchSize = 3000) => {
			...
			const processNextBatch = () => {
				...
				while (currentRow <= rowCount && cellsInBatch < batchSize) {
					...
					while (currentCol <= colCount && cellsInBatch < batchSize) {
						...
						// 使用批量数据收集替代直接更新
						batchData.push({
							r: currentRow - 1 + startRow,
							c: currentCol - 1 + startCol,
							v: mergeInfo
								? mergeInfo.isStart
				...
				if (batchData.length > 0) {
					batchData.forEach((item) => {
						sheet.hooks.editHook.setCellValue(item.r, item.c, item.v, true)
					})
	...
	const readExcelFile = async (file) => {
		...
		try {
			sheet.state.importing = true
			sheet.state.loading = true
			sheet.state.loadingText = '正在导入Excel文件...'
			...
			sheet.config.locked = {}
			sheet.config.styled = {}
			sheet.hooks.resizeHook.clear()
			sheet.hooks.mergeHook.clearMergedCells()
			...
			sheet.state.importing = false
			sheet.state.loading = false
			sheet.state.loadingText = ''
