...
const setOffsetHeight = (flow) => {
	...
	if (props.height) {
		...
		while (
			parent &&
			parent.tagName !== 'HTML' &&
			!parent.classList.contains('container-body')
		) {
...
const onAddConditionByGateway = (props) => {
	getNodeConfigById(nodeConfig.value, props.id).then((node) => {
		const conditionCount = toObject().nodes.filter(
			(f) => f.type === FlowNodeTypes.Condition
		).length
		...
		if (!curNode.childNode || curNode.childNode?.length === 0) {
			...
			if (curNode.type === FlowNodeTypes.Condition) {
				...
				if (curParentNode.conditionNodes.length > 2) {
					curParentNode.conditionNodes = curParentNode.conditionNodes.filter(
						(f) => f.id !== id
					)
					resetNode()
				} else {
					getNodeConfigById(nodeConfig.value, curParentNode.pid).then(
						(gatewayParentNode) => {
							const firstConditionChildNode =
								curParentNode.conditionNodes[0].childNode
